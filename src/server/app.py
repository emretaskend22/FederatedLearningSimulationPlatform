...
@app.post("/update")
async def update(client_id: str = Form(...), num_samples: int = Form(...), loss: float = Form(0.0), accuracy: float = Form(0.0), dp_epsilon: float = Form(0.0), file: UploadFile = File(...)):
    content = await file.read()
    buffer = io.BytesIO(content)
    state_dict = torch.load(buffer)
    
    metrics = {"loss": loss, "accuracy": accuracy}
    round_complete = server.receive_update(client_id, state_dict, num_samples, metrics, dp_epsilon=dp_epsilon)
