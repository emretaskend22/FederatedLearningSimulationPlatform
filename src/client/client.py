    ...
    def train(self, config, privacy_engine=None):
        ...
        if privacy_engine:
             with torch.no_grad():
                ...
                total_norm = torch.sqrt(sum(((state[k] - global_state[k]) ** 2).sum() for k in state))

                C = float(privacy_engine.sensitivity)
                clip_factor = min(1.0, C / (total_norm.item() + 1e-12))

                for k in state:
                    delta = (state[k] - global_state[k]) * clip_factor
                    noisy_delta = privacy_engine.apply(delta)
                    state[k].copy_(global_state[k] + noisy_delta)
                print(f"[DP DEBUG] sigma={privacy_engine.sigma:.6f} ||delta||={total_norm.item():.6f} clip_factor={clip_factor:.6f}")
